...
public class GenericController implements Controller {
	...
	private ResponseVO callBusinessMethod(HttpRequest httpRequest)
		...
		if (Constants.HTTP_METHOD_POST.equalsIgnoreCase(httpMethod)) {
			...
			List<Method> methodList = getMethod(POST.class);
			...
			ResponseVO responseVO = callMethod(method, httpRequest,content);
			return responseVO;
			// Call Get method
			...
			if (methodList.size() > 1) {
				return prepareResponse(Constants.RESPONSE_TYPE_ERROR, "Non Supported Method server side error");
			}
			Method method = methodList.get(0);

			ResponseVO responseVO = callMethod(method, httpRequest,null);
			return responseVO;
			// Call Get method
	...
	private ResponseVO callMethod(Method method, HttpRequest httpRequest,String content)
